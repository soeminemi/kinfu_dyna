...
class PredictionModel(nn.Module):
    def __init__(self, input_size=88, hidden_size=128, output_size=22, num_residual_blocks=3):
        ...
        # 底层交叉注意力
        self.low_level_attention = CrossAttentionLayer(hidden_size, num_heads=4)
    ...
    def forward(self, x):
        ...
        # 特征提取
        point_features = self.point_feature(target)  # (batch_size, hidden_size * 2)
...
def main():
    ...
    if mode == 'train':
        ...
        }, 'hw_prediction_model_nm.pth')
        print("\nModel and standardizer saved to 'hw_prediction_model_nm.pth'")
    elif mode == 'test':
        # Testing process
        test_model(model, test_loader, device, 'hw_prediction_model_nm.pth')
    else:
