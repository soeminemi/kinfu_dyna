...
class PredictionModel(nn.Module):
    def __init__(self, input_size=88, hidden_size=128, output_size=22, num_residual_blocks=3):
        ...
        self.fusion_layer = nn.Sequential(
            nn.Linear(hidden_size+22, hidden_size),  
            nn.LayerNorm(hidden_size),  
            nn.LeakyReLU(0.2),
            nn.Dropout(0.2)
        )
    ...
    def forward(self, x):
        ...
        # 输出
        x = self.final_layer(x) + target # + self.bias_factor
        return x
...
def load_and_prepare_data():
    ...
    train_measurements = np.hstack((train_data[:, :22], train_data[:, -1].reshape(-1, 1)))
    val_measurements = np.hstack((val_data[:, :22], val_data[:, -1].reshape(-1, 1)))
    test_measurements = np.hstack((test_data[:, :22], test_data[:, -1].reshape(-1, 1)))
...
def test_model(model, test_loader, device, checkpoint_path):
    ...
    for i in range(len(all_predictions)):
        ...
        for dim in range(len(all_predictions[i])):
            print(f"{'dim':<15}{all_predictions[i][dim]:<20}{all_targets[i][dim]:<20}{all_deviations[i][dim]:<20}")
        print("\n")  # New line for separation between samples
    # Calculate MAE for all samples
    mae_all_samples = np.mean(np.abs(all_deviations), axis=1)
    
    print("\nMAE for all samples:")
    print(f"{'Sample':<15}{'MAE':<10}")
    print("="*25)
    # for i, mae in enumerate(mae_all_samples):
    #     print(f"{sample_names[i]:<15}{mae:.4f}")
    
    print(f"\nOverall MAE: {np.mean(mae_all_samples):.4f}")
...
def main():
    ...
    if mode == 'train':
        ...
        # Testing process
        test_model(model, train_loader, device, 'hw_prediction_model.pth')
    else:
